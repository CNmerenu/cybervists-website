...
export default function Services() {
  return (
    <section
      id="services"
      className="w-full md:w-[1440px] py-16 md:py-24 bg-primary-400 relative overflow-hidden"
    >
      ...
      <div className="px-4 md:px-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
          {services.map((service, index) => (
            <div
              key={index}
              className="bg-accent-100 border border-blue-500/30 rounded-xl p-4 md:p-8 hover:border-blue-500/50 transition-all duration-300 hover:shadow-xl group"
            >
              <div className="w-32 h-32 mb-4 mx-auto bg-white rounded-full overflow-hidden border-none border-primary-700 relative">
                <Image
                  src={service.image}
                  alt={service.title}
                  fill
              ...
              <Link href={"/#contact"}>
                <h3 className="text-lg md:text-2xl font-bold text-black mb-3 md:mb-4 transition-colors text-center cursor-pointer">
                  {service.title}
                </h3>
              </Link>
              <p className="text-sm md:text-base text-primary-500 leading-relaxed text-center">
                {service.description}
              </p>
            </div>
          ))}
